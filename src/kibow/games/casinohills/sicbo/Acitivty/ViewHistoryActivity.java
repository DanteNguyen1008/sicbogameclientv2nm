...
public class ViewHistoryActivity extends BaseActivity implements
		OnClickListener {

	TableLayout tblHistory;
	RelativeLayout rela;
	boolean isSpace = false;
	int size, nextIndexHistory, backIndexHistory, indexLastDate, countLoad,
			countBetDate;
	...
	private void fillRow() {
		...
		for (int i = indexLastDate; i < size; i++) {
			...
			winPattern = GameEntity.getInstance().currentGame
					.convertStringtoArrayList(historyGame.get(i).betSpot);
			addTextToRowWithValues(headerRow, winPattern.get(0).toString(),
					textColor, textSize);
			...
			for (int j = 0; j < lenght - 1; j++) {
				...
				addTextToRowWithValues(headerRow, "", textColor, textSize);
				addTextToRowWithValues(headerRow, "", textColor, textSize);
				addTextToRowWithValues(headerRow, winPattern.get(j + 1)
						.toString(), textColor, textSize);
				tblHistory.addView(headerRow);
			}
	...
	public void onClick(View arg0) {
		...
		case R.id.btn_load_history: {
			...
			if (countBetDate > 19) {
				...
				paramsName.add("last_date");
				ArrayList<Object> paramsValue = new ArrayList<Object>();
				paramsValue.add(historyGame.get(size - 1).betDate);
				Object[] params = { GameEntity.getInstance().connectionHandler,
						this, GameEntity.VIEW_HISTORY, paramsName, paramsValue };
	...
	public void onReceiveViewHistory(JSONObject result, Activity activity)
		...
		if (numOfItem > 0) {
			for (int i = 0; i < numOfItem; i++) {
				historyGame.add(new HistoryComponent(result.getJSONObject(
						i + "").getBoolean("iswin"), result.getJSONObject(
						i + "").getString("betdate"), result.getJSONObject(
						i + "").getDouble("balance"), result.getJSONObject(
						i + "").getString("dices"), result
