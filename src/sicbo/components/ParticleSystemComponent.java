...
public class ParticleSystemComponent extends AbItemComponent {
	...
	public ParticleSystemComponent(int id, final int width, final int height,
			...
			final float positionY, final TextureManager textureManager,
			final Context context, final Engine engine, ItemType itemType,
			Color color, int mNumPart, int mTimePart, Scene scene) {
		...
		this.scene = scene;
		particleEmitter = new PointParticleEmitter(
				positionX, positionY);
		...
		IEntityFactory<Sprite> recFact = new IEntityFactory<Sprite>() {
			...
			public Sprite create(float pX, float pY) {
				...
				Sprite sprite = new Sprite(positionX, positionY,
						atlasRegionBig, engine.getVertexBufferObjectManager());
		...
		this.scene.registerUpdateHandler(new TimerHandler(mTimePart,
				new ITimerCallback() {
					...
					public void onTimePassed(final TimerHandler pTimerHandler) {
						...
						ParticleSystemComponent.this.scene.unregisterUpdateHandler(pTimerHandler);
					}
				}));
	}
